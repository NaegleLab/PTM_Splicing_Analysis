...
def constructPivotTable(ptms, regulatory, reference_col = 'ON_FUNCTION', collapse_on_similar = False, include_unknown = False):
    """
...
def get_enriched_annotations(ptm_subset, reference_df, psp_annotations, subset_name = 'Subset'):
    """
    Given a subset of ptms, perform enrichment analysis using both molecular function annotation and biological process annotation
    """
    #process annotations
    process_table = constructPivotTable(reference_df, psp_annotations, reference_col = 'ON_PROCESS', collapse_on_similar=True)
    process_enrichment = generate_site_enrichment(ptm_subset, process_table, subset_name = subset_name)

    #function annotations
    function_table = constructPivotTable(reference_df, psp_annotations, reference_col = 'ON_FUNCTION', collapse_on_similar=True)
    function_enrichment = generate_site_enrichment(ptm_subset, function_table, subset_name = subset_name)

    return process_enrichment, function_enrichment